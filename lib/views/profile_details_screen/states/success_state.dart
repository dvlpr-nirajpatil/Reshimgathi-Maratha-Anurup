...
class ProfileDetailScreenSuccessState extends StatefulWidget {
  ProfileDetailScreenSuccessState({super.key, required this.data});
  ...
  @override
  State<ProfileDetailScreenSuccessState> createState() =>
      _ProfileDetailScreenSuccessStateState();
}

class _ProfileDetailScreenSuccessStateState
    extends State<ProfileDetailScreenSuccessState> {
  // ignore: prefer_typing_uninitialized_variables
  var swiper;
  ...
  initState() {
    super.initState();
    widget.brothersList = widget.data['brothers_info'];
    widget.sistersList = widget.data['sisters_info'];
    widget.mamaList = widget.data['mamas_info'];
    fetchRequestStatus();
  }

  initSwiper() {
    var controller = Provider.of<ProfileScreenController>(context);
    swiper = VxSwiper(
  ...
  fetchRequestStatus() async {
    ...
    requestStatus = await controller.fetchRequestStatus(
        requestedProfile: widget.data['uid']);
    setState(() {});
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      bottomNavigationBar:
          Consumer<RequestInfoController>(builder: (context, controller, xxx) {
        return BottomAppBar(
            elevation: 0,
      ...
      appBar: AppBar(
        title: "${widget.data['first_name']} ${widget.data['last_name']}"
            .text
            .make(),
        ...
        actions: [
          const Icon(
            ...
            () {
              Share.share(
                  "Check out this profile https://reshimgathi.probity.dev/home/profile/${widget.data['uid']}");
            },
          ),
          10.widthBox,
        ],
